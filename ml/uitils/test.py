...
def test_extract_text_from_file(file,prompt="Solve it well please",user_id="user_1"):
    ...
    retrived= retrieval(f"The user gave prompt{prompt} and asks you to solve {text}",params={"user_id":user_id})
    subject = retrived.get("subject", [])
    chapter = retrived.get("chapter", [])
    ...
    text=generate_openai(f"""
                         ...
                         """,max_tokens=5000,json_parse=True)
    

    return text
    # response=process_query(f"Solve the assignment given to the user in the file. Extra prompt from the user: {prompt}. Extracted text = {text}",{"user_id":"user_1"})
