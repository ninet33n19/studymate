...
def generate_openai(prompt,max_tokens=2000,temperature=0.3,model='gpt-4o-mini',json_parse=False):
    ...
    if json_parse and response.choices[0].message.content is not None:
...
def process_query(query_text, params):
    ...
    generated_text = generate_openai(f"{prompt}. Only reply in plaintext and not markdown.", max_tokens=5000, temperature=0.3)
