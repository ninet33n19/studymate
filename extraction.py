...
def extract_text_from_file(file, mimetype=None):
    """
    ...
    if mimetype == "application/pdf":
        ...
        extracted_text = get_description_from_image(file)
...
def extract_syllabus_or_date_changes(extracted_text,user_id):
    ...
    user_profile = users_collection.find_one({"user_id": user_id})
    syllabus = json.dumps(user_profile['syllabus'])
    calendar = json.dumps(user_profile['calendar'])
    ...
    result = response.choices[0].message.content.replace("```json","").replace("```","")
    print(result)
    return json.loads(result)  # Assuming JSON response for syllabus or date changes
